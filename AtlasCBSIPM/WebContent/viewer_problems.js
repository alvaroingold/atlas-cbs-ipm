...
//
//
// Latest changes by C.A-Z
// during his stay at UAH
// Feb.-June, 2013.
...
var current_x = 4;
var current_y = 6;
var lei_labels = ["LE","BEI","SEI","NSEI","NBEI","nBEI","mBEI","LEP"];
    ...
    var haspdb = "";
	  ...
	  if( text_split.length > 2){
	    var secondpart = text_split[2];
            txtsplit3 = secondpart.split("#");
	    pdb=txtsplit3[1];
	    haspdb='<TR><TD>PDB</TD><TD>'+pdb+'(<A target=\"_blank\" HREF=\"http://www.pdb.org/pdb/explore/explore.do?structureId='+pdb+'\">RCSB</A> | <A target=\"_blank\" HREF=\"http://www.ebi.ac.uk/pdbe-srv/view/entry/'+pdb+'/summary.html\">PDBe</A>)</TD></TR>';
	  }
...
// Selection of the compound from the PDB. Cele & Peter Rose request on end April 2012
	...
	update_selection_graph();
       ...
       var $dialog = $('<div></div>')
                .html('<p>Sorry! This PDB code could not extract any affinity data from BindingDB. Consult with <A HREF="http://www.bindingdb.org/">BindingDB</A> or <A HREF="http://www.pdb.org">PDB</A> before you try another one. Make sure that it corresponds to a target-ligand pair PDB entry within PDBBind.</p>')
                .dialog({
                        autoOpen: true,
                        title: 'PDB code',
                        resizable: true,
...
function check_ic50_and_leh()
{
      ...
      if( ds.options.length > 0)
      {
              ...
              for( i = 0; i < ds.options.length; ++i)
              {
                ph=trim(ds.options[i].value).split("#")[2]
		if ( ph == "3")
		 flag = 1;
              }
      }
	...
	{
       var $dialog = $('<div></div>')
                .html('<p>Hopkins Ligand efficiency (LE) is not available for IC50 datasets. LE values set to 0.0</p>')
                .dialog({
                        autoOpen: true,
                        title: 'Warning',
                        resizable: true,
...
function addSource()
{
 ...
 if ( idp == "3" && (current_x == "1" || current_y == "1"))
   check_ic50_and_leh();
...
{
//        var $dialog = $('<div></div>')
//                .html('<p>Currently these are the definitions of the Ligand Efficiency Indices (LEI) used in this platform:</p> <CENTER><table width="350" align= "center" border="1" RULES="ROWS" FRAME="HSIDES" cellspacing="0" cellpadding="2" style="font-size: 0.7em;"> <tr> <td><p><strong>Name</strong></p></td>  <td><p><strong>Definition</strong></p></td> </tr> <tr> <td><p>BEI</p></td> <td><p>p(Ki), p(Kd), or p(IC<sub>50</sub>)/MW(kiloDa) </p></td> </tr> <tr> <td><p>SEI</p></td><td><p>p(K<sub>i</sub>), p(K<sub>d</sub>), or p(IC<sub>50</sub>)/(PSA/100 &Aring;<sup>2</sup>)</p></td><tr><td><p>NSEI</p></td><td><p>NSEI = -log<sub>10</sub> Ki/(NPOL) = pKi/NPOL(N,O)</p></td><tr><td><p>NBEI</p></td> <td><p>NBEI= -log<sub>10</sub> Ki/(NHA)= pKi/(NHA)</p></td> </tr <tr><td><p>nBEI</p></td<td><p>nBEI= -log<sub>10</sub>[(Ki/NHA)]</p></td></tr><tr><td><p>mBEI</p></td><td><p>mBEI=-log<sub>10</sub>[(Ki/MW)]</p></td> </tr></table></CENTER><p>More information can be found <A HREF="http://www.uic.edu/labs/caz/discovery/index.html">here</A></p>')
var $dialog = $('<div></div>').html('<ul><li>LE(LEH),BEI,NBEI, nBEI and mBEI are efficiency indices related to the size of the ligand. LEH,LEP are calculated only if K<sub>i</sub> (or K<sub>d</sub>) are available; otherwise, LEH,LEP =0.0 </li><li>SEI,NSEI,LEP are efficiency indices relating to the polarity of the molecules.</li></ul><hr><ul></li><li>LEH,LEP are the DeltaG divided by NHA (Hopkins et al. DDT 2004), NPOL (CAZ) respectively.</li><li>BEI is the pKi scaled to the MW in KDaltons (KiloDa).</li><li>NBEI is the pKi scaled to the number of non-hydrogen atoms (NHA).</li><li> For nBEI, mBEI the log<sub>10</sub> operation is taken after the ratio of K<sub>i</sub> to the scaling factor (NHA, MW) respectively. This might appear to be odd but the algebra is such that the slope of the lines in these planes is easier to interpret as the NPOL number. Note: various size-related efficiency indices can be easily inter-converted by scale factors. </li></ul> <hr> <ul><li>SEI is pK<sub>i</sub> scaled down to the Polar Surface Area (normalized to 100 &Aring;<sup>2</sup>).</li><li> NSEI is pK<sub>i</sub> divided by the number of polar atoms (N+O).
</li><li> LEP is an extension of Hopkins LE (or LEH) and defined as DeltaG divided by NPOL(N+O). </li></ul><hr>Efficiency planes combine the two types of variables, typically BEI(group) vs. SEI(group) (y vs. x).<p>Variable names:</p><ul><li>NHA: number of non-hydrogen atoms in the ligand</li><li>PSA (or tPSA): Polar Surface Area (or topological Polar Surface area)</li><li>NPOL: number of Polar Atoms,  the count of N+O atoms in the ligand</li><li>MW: Molecular Weight, in the BEI definition should be in KiloDaltons</li></ul><p>More information can be found <A HREF="http://www.uic.edu/labs/caz/discovery/index.html">here</A></p>')
                .dialog({
                        autoOpen: true,
