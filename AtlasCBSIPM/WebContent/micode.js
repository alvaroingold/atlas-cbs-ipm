...
This stuff is supposed to extract the users targets, organism and data only
if we have auth.
...
function targetOnChange(db){

var stillworking=document.getElementById("mystatus");
stillworking.innerHTML="<img src='img/loading.gif' align='absmiddle'>";
var requestUrl;
requestUrl = "/atlas/servlets/servlet/LEIgetTarget?database=" + encodeURIComponent(db);
CreateXmlHttpObj();
if(XmlHttpObj)
{
...
function StateChangeHandler_back()
{
	if(XmlHttpObj.readyState == 4)
	{
		if(XmlHttpObj.status == 200)
